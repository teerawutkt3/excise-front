...
import { BreadCrumb, ResponseData, License, LicenseAlcohol } from 'models/index';
...
import { Province, Amphures, Districts, Zipcodes, ProvinceService } from 'services/province.service';
import { Ope020701CustomerVo } from 'projects/operation-audit/ope02/ope0207/ope020701/ope020701vo.model';

declare var $: any;

const URL = {
     GET_FIND: "oa/04/12/find/customerLicense",
     POST_SAVE: "oa/04/12/save/customerLicense",
     POST_UPDATE: "oa/04/12/update/customerLicense",
...
@Component({
     selector: 'app-ope04-12-02',
     templateUrl: 'ope041202.component.html',
     styleUrls: ['ope041202.component.css']
})
export class Ope041202Component {
     breadcrumb: BreadCrumb[] = [
          { label: "ตรวจปฏิบัติการ", route: "#" },
          { label: "น้ำมันหล่อลื่น", route: "#" },
          { label: "ข้อมูลผู้ได้รับอนุญาตผลิตสุรา", route: "#" },
          { label: "เพิ่มข้อมูลผู้ได้รับอนุญาตผลิตสุรา", route: "#" },
     ];
     // Message
     ...
     constructor(
          ...
          this.form = this.fb.group({
               ...
               licenseTypeFor: [null,Validators.required],
               licenseTypeDesp: [null,Validators.required],
               bankGuarantee: ['N'],
          ...
          this.companyCapa = ["เล็ก", "กลาง", "ใหญ่"];
     }
     ...
     ngAfterViewInit() {
          ...
          $('#usedDate').calendar({
               ...
               formatter: formatter('วดป'),
               onChange: (date, text) => {
                    this.form.get('usedDate').patchValue(date);
               }
          });
     ...
     save() {
          let data: LicenseAlcohol = this.form.value;
          this.submitted = true;
     ...
     find() {
          this.ajax.doGet(`${URL.GET_FIND}/${this.id}`).subscribe((response: ResponseData<LicenseAlcohol>) => {
               if (MessageService.MSG.SUCCESS == response.status) {
                    // this.details = this.form.get('details') as FormArray;
                    // for (let i = 0; i < response.data.details.length; i++) {
                    //      if (this.details.at(i)) {
                    ...
                    this.form.patchValue({
                         ...
                         // oldCustomer: response.data.oldCustomer,
                         mobile: response.data.mobile,
                         warehouseAddress: response.data.licenseAddress,
                         addressNo: response.data.address.split(" ")[0].replace("เลขที่", "").trim(),
                         district: district,
                         amphoe: amphoe,
                         province: province,
     ...
     onFocus(event) {
          ...
          this.form.get('money').patchValue(event.target.value.replace(/,/g, ''));
     }

     onBlur(event) {
          // this.details = this.form.get('details') as FormArray;
          // this.details.at(index).get('amount').patchValue(new DecimalFormatPipe().transform(event.target.value, "###,###"));
          this.form.get('money').patchValue(new DecimalFormatPipe().transform(event.target.value, "###,###"));
     }

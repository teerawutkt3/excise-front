...
import { Ope04010604Vo } from './ope04010604vo.model';
...
const URL = {
  ...
  GET_DETAILS: "oa/04/01/06/04/detail",
  PUT_UPDATE: "oa/04/01/06/04/save",
};

@Component({
  selector: 'app-ope04010604',
  templateUrl: './ope04010604.component.html',
  styleUrls: ['./ope04010604.component.css']
})
export class Ope04010604Component implements OnInit {
  loading: boolean = false;
  id: string = "";
  descriptions: any[] = [];
  forms: FormGroup = new FormGroup({});
  datas: FormArray = new FormArray([]);
  request: RequestOpe04010604 = {
    save: [],
    delete: []
  };
  ...
  constructor(
    ...
    this.descriptions = [
      { name: "ชนิดวัตถุดิบหลัก", content: "วัตถุดิบหลักที่ใช้ เช่น ข้าวเหนียว กระเจี๊ยบ องุ่น เป็นต้น" },
      ...
      { name: "ยอดคงเหลือ", content: "จำนวนของวัตถุดิบที่คงเหลือในวันนั้น โดยนำยอด คงเหลือของวันก่อนหน้ามาบวกกับจำนวนรับ แล้วหักด้วยจำนวนจ่าย" },
    ];
  }
  ...
  getDetail(): void {
    this.loading = true;
    this.ajax.doGet(`${URL.GET_DETAILS}/${this.buttons.oaAlcoholDtlId}`).subscribe((response: ResponseData<Ope04010604Vo[]>) => {
      if (MessageService.MSG.SUCCESS == response.status) {
  ...
  submit(): void {
    this.loading = true;
    if (this.forms.valid) {
      this.request.save = this.forms.value.datas as Ope04010604Vo[];
      ...
      this.ajax.doPut(`${URL.PUT_UPDATE}`, this.request).subscribe((response: ResponseData<RequestOpe04010604>) => {
        if (MessageService.MSG.SUCCESS == response.status) {
  ...
  onAddField(): void {
    ...
    this.request.save = this.datas.value as Ope04010604Vo[];
    setTimeout(() => {

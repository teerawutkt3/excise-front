...
import { Ope0208Vo, Ope0208ApproveVo } from './ope0208vo.model';

declare var $: any;

const URL = {
  GET_FIND: "oa/02/08/find",
  GET_FIND_APPROVE: "oa/02/08/findApprove",
}

@Component({
  selector: 'app-ope0208',
  templateUrl: './ope0208.component.html',
  styleUrls: ['./ope0208.component.css']
})
export class Ope0208Component implements OnInit {
  ...
  getByBudgetYear() {
    // WAIT FOR APPROVE
    this.ajaxService.doGet(`${URL.GET_FIND}/${this.form.get('budgetYear').value}`).subscribe((response: ResponseData<Ope0208Vo[]>) => {
    ...
    // APPROVED
    this.ajaxService.doGet(`${URL.GET_FIND_APPROVE}/${this.form.get('budgetYear').value}`).subscribe((response: ResponseData<Ope0208ApproveVo[]>) => {
      if (MessageService.MSG.SUCCESS == response.status) {
  ...
  redirectTo(id: number) {
    this.router.navigate(['/ope02/08/01'], {
      queryParams: {
        id: id
      }
    });

...
import { Ope020102Vo, Ope0201001Vo, Ope020102Store } from 'projects/operation-audit/ope02/ope0201/ope020102/ope020102.model';
import { Ope020103Vo, Ope020103Store } from 'projects/operation-audit/ope02/ope0201/ope020103/ope020103.model';
import { Observable } from 'rxjs';
import { FormGroup, FormBuilder } from '@angular/forms';
import { MessageBarService } from 'services/index';
import { AjaxService } from 'services/index';
import { Store } from '@ngrx/store';
import { Ope0201FromVo, Ope0201Vo, Ope0201 } from 'projects/operation-audit/ope02/ope0201/ope0201Vo.model';
...
const URL = {
  ...
  GET_PLAN_DETAIL :"oa/02/01/findOaPlanHyd",
  GET_APPROVER :"oa/02/01/findApprover",
  GET_SENDAPPROVE :"oa/02/01/sendApprove",
}

@Component({
  selector: 'app-ope0101',
  templateUrl: './ope0101.component.html',
  styleUrls: ['./ope0101.component.css']
})
export class Ope0101Component implements OnInit {
    ...
    this.ajax.doGet(`${URL.GET_PLAN_DETAIL}/${this.planId}`).subscribe((response: ResponseData<Ope0201FromVo>) => {
      if (MessageService.MSG.SUCCESS == response.status) {
        ...
        auditerList.forEach(element => {
          let auditer:Ope020103Vo = this.initAuVo();
          auditer.userThaiName = element.userThaiName;
          auditer.title = element.title;
          auditer.wsUserId = element.wsUserId;
          auditer.oaPersonAuditPlanId = element.oaPersonAuditPlanId;
          auditStore.auditer.push(auditer);
        });
  ...
  gotoFindCustomer() {
    ...
    // this.router.navigate(['/ope01/01/02']);
    this.router.navigate(['/ope01/01/02'], {
      queryParams: {
        to: this.planStatus,
        planId:this.planId
  ...
  gotoFindUserAudit() {
    this.store.dispatch(new OPE0201ACTION.UpdateOpe0201(this.ope0201));
    // this.router.navigate(['/ope01/01/03']);
    this.router.navigate(['/ope01/01/03'], {
      queryParams: {
        to: this.planStatus,
        planId:this.planId

...
import { BreadCrumb, ResponseData, LicenseHydrocarbon } from 'models/index';
...
import { Ope010701CustomerVo } from './ope010701vo.model';

declare var $: any;

const URL = {
     GET_FIND: "oa/01/07/find/customerLicense",
     POST_SAVE: "oa/01/07/save/customerLicense",
     POST_UPDATE: "oa/01/07/update/customerLicense",
     GET_CUSTOMER: "oa/01/07/find/customer",
     GET_CUSTOMER_LIST: "oa/01/07/find/customers"
}

@Component({
     selector: 'app-ope01-07-01',
     templateUrl: 'ope010701.component.html',
     styleUrls: ['ope010701.component.css']
})
export class Ope010701Component {
     breadcrumb: BreadCrumb[] = [
          { label: "ตรวจปฏิบัติการ", route: "#" },
          { label: "สารละลายประเภทไฮโดรคาร์บอน", route: "#" },
          { label: "ข้อมูลผู้ประกอบการสารละลายประเภทไฮโดรคาร์บอน", route: "#" },
          { label: "เพิ่มข้อมูลผู้ได้รับอนุญาต (ผู้ใช้/ตัวแทน) สารละลายประเภทไฮโดรคาร์บอน", route: "#" },
     ];
     // Message
     ...
     save() {
          let data: LicenseHydrocarbon = this.form.value;
          this.submitted = true;
          if (this.form.valid) {
               let data: LicenseHydrocarbon = this.form.value;
               ...
               this.ajax.doPost(URL.POST_SAVE, data).subscribe((response: ResponseData<LicenseHydrocarbon>) => {
                    if (MessageService.MSG.SUCCESS == response.status) {
                         this.msg.successModal(response.message, "สำเร็จ", event => {
                              if (event) {
                                   ...
                                   //      bankGuaranteeTxt: response.data.bankGuaranteeTxt,
                                   //      licenseTypeUsr: response.data.licenseTypeUsr,
                                   //      licenseTypeUsrATxt: response.data.licenseTypeUsrATxt,
                                   //      licenseTypeUsrBTxt: response.data.licenseTypeUsrBTxt,
                                   ...
                                   // }
                                   // this.breadcrumb[3].label = "แก้ไขข้อมูลผู้ได้รับอนุญาต (ผู้ใช้/ตัวแทน) สารละลายประเภทไฮโดรคาร์บอน";
                                   // this.id = response.data.oaCuslicenseId.toString();
                                   // this.state = "EDIT";
                                   // setTimeout(() => {
                                   //      this.router.navigate(['/ope01/07/01'], {
                                   //           queryParams: {
                                   //                id: this.id,
                                   //                state: "EDIT"
                                   //           }
     ...
     update() {
          ...
          if (this.form.valid) {
               ...
               this.ajax.doPut(URL.POST_UPDATE, data).subscribe((response: ResponseData<LicenseHydrocarbon>) => {
                    if (MessageService.MSG.SUCCESS == response.status) {
                         this.msg.successModal(response.message, "สำเร็จ", event => {
                              if (event) {
                                   this.details = this.form.get('details') as FormArray;
     ...
     find() {
          this.ajax.doGet(`${URL.GET_FIND}/${this.id}`).subscribe((response: ResponseData<LicenseHydrocarbon>) => {
               if (MessageService.MSG.SUCCESS == response.status) {
                    this.details = this.form.get('details') as FormArray;
                    for (let i = 0; i < response.data.details.length; i++) {
                         if (this.details.at(i)) {
                    ...
                    if (this.stateView) {
                         this.breadcrumb[3].label = "รายละเอียดผู้ได้รับอนุญาต (ผู้ใช้/ตัวแทน) สารละลายประเภทไฮโดรคาร์บอน";
                    } else {
                         this.breadcrumb[3].label = "แก้ไขข้อมูลผู้ได้รับอนุญาต (ผู้ใช้/ตัวแทน) สารละลายประเภทไฮโดรคาร์บอน";
                    }
                    this.setProAmpDis(province, amphoe, district);
               } else {
                    this.msg.errorModal(response.message);
               }
     ...
     onCustomerChange(event) {
          if (event.target.value) {
               this.ajax.doGet(`${URL.GET_CUSTOMER}/${event.target.value}`).subscribe((response: ResponseData<LicenseHydrocarbon>) => {
                    if (MessageService.MSG.SUCCESS == response.status) {
                         const province: string = response.data.address.split(" ")[3].replace("จ.", "").trim();
                         const amphoe: string = response.data.address.split(" ")[2].replace("อ.", "").trim();
                         const district: string = response.data.address.split(" ")[1].replace("ต.", "").trim();
     ...
     setProAmpDis(province: string, amphoe: string, district: string): any {
          setTimeout(() => {
               $('#province').dropdown('set selected', province);
               const indexA: number = this.provinces.findIndex(obj => obj.province_name == this.form.get('province').value);
               if (indexA != -1) {
                    this.amphures = this.provinceService.findByProvince(this.provinces[indexA].province_id);
               }
               ...
               setTimeout(() => {
                    ...
                    const indexB: number = this.amphures.findIndex(obj => obj.amphur_name == this.form.get('amphoe').value);
                    if (indexB != -1) {
                         this.districts = this.provinceService.findByAmphure(this.amphures[indexB].amphur_id);
                    }
                    ...
                    setTimeout(() => {
                         ...
                         const indexC: number = this.districts.findIndex(obj => obj.district_name == this.form.get('district').value);
                         if (indexC != -1) {
                              this.zipcodes = this.provinceService.findByDistrictCode(this.districts[indexC].district_code);
                         }
                         this.form.get('postcode').reset();
     ...
     findCustomers() {
          this.ajax.doGet(`${URL.GET_CUSTOMER_LIST}`).subscribe((response: ResponseData<Ope010701CustomerVo[]>) => {
               if (MessageService.MSG.SUCCESS == response.status) {
     ...
     onChangeLicenseType(event: any) {
          if (event.target.value == "A") { // User Case
               this.onChangeLicenseTypeUsr({ target: { value: "A" } });
          } else { // Agent Case
               // Clear Text
               this.onChangeLicenseTypeUsr({ target: { value: "C" } });
          }
     }

     onChangeLicenseTypeUsr(event: any) {
          if (event.target.value == "A") { // User A Case
               this.form.get('licenseTypeUsrATxt').patchValue('');
               this.form.get('licenseTypeUsrBTxt').patchValue('');
          } else if (event.target.value == "B") { // User B Case
               this.form.get('licenseTypeUsrATxt').patchValue('');
               this.form.get('licenseTypeUsrBTxt').patchValue('');
          } else { // Agent other case
               this.form.get('licenseTypeUsr').patchValue('');
               this.form.get('licenseTypeUsrATxt').patchValue('');
               this.form.get('licenseTypeUsrBTxt').patchValue('');
          }
     }

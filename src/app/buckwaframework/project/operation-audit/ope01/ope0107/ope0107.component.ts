...
import { TypeOfCompany, TypeOfLicense } from './ope0107.mock';
import { DateStringPipe } from 'app/buckwaframework/common/pipes';

declare var $: any;

const URL = {
  SEARCH: AjaxService.CONTEXT_PATH + "oa/01/07/filter",
  POST_SECTOR_LIST: "preferences/department/sector-list",
  POST_AREA_LIST: "preferences/department/area-list",
}

@Component({
  selector: 'app-ope0107',
  templateUrl: './ope0107.component.html',
  styleUrls: ['./ope0107.component.css']
})
export class Ope0107Component implements OnInit {
  breadcrumb: BreadCrumb[] = [
    { label: "ตรวจปฏิบัติการ", route: "#" },
    { label: "สารละลายประเภทไฮโดรคาร์บอน", route: "#" },
    { label: "ข้อมูลผู้ได้รับอนุญาต (ผู้ใช้/ตัวแทน) สารละลายประเภทไฮโดรคาร์บอน", route: "#" },
  ];
  offCode: string = "";
  dataTable: any = null;
  form: FormGroup = null;
  ...
  ngAfterViewInit() {
    if (this.isMain) {
      ...
      setTimeout(() => $('#area').dropdown('set selected', this.offCode), 1000);
